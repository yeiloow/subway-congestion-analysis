...
with tab1:
    ...
    with col_t1_1:
        ...
        st.plotly_chart(fig_usage_count, use_container_width=True)

    with col_t1_2:
        st.subheader("2. 건물 용도별 총 연면적")
        usage_area = filtered_df.groupby("usage_type")["total_area"].sum().reset_index()
        ...
        st.plotly_chart(fig_usage_area, use_container_width=True)
